  ...
  <div class="task-page">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="creatorName" label="发起人" width="160" />
        <el-table-column prop="currentVersion" label="版本号" width="100">
        ...
        <el-table-column prop="pendingDays" label="待审天数" width="120">
          <template #default="{ row }">
            {{ row.pendingDays != null ? row.pendingDays + ' 天' : '-' }}
          </template>
        </el-table-column>
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              type="success"
              link
              :disabled="row.currentStatus !== 'REVIEWING'"
...
import { fetchReviewerTasks, getTaskDetail } from '@/api/task'
...
const loadTasks = async () => {
  loading.value = true
  try {
    const page = await fetchReviewerTasks(buildParams())
...
const handleStartReview = async (row) => {
  if (row.currentStatus !== 'REVIEWING') {
    return
  }
  let versionId = row.currentVersionId
  if (!versionId) {
    try {
      const detail = await getTaskDetail(row.taskId)
      versionId = detail?.currentVersionDetail?.versionId
    } catch (error) {
      ElMessage.error(error?.message || '无法获取任务详情')
      return
    }
    if (!versionId) {
      ElMessage.error('未获取到当前版本信息，请稍后再试')
      return
    }
  }
  router.push({
